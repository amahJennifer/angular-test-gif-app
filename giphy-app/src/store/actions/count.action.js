...
export const fetchAllGifs = (query, limit = 25, offset = 0) => {
	return async (dispatch) => {
		try {
			dispatch(fetchStart(true));
			const res = await axios.get(
				`https://api.giphy.com/v1/gifs/search?api_key=deokzgUjxm6QHQdp3H3aca1LSZcCpucc&q=${query}&limit=${limit}&offset=${offset}&rating=G&lang=en`
			);
